...
<style>
    ...
    .trade-card { background-color: #181a20; padding: 20px; border-radius: 8px; border: 1px solid #2b3139; box-shadow: 0 4px 8px rgba(0,0,0,0.4); font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
    ...
    .text-whale { color: #0ea5e9 !important; font-weight: bold; text-shadow: 0 0 5px rgba(14,165,233,0.3); }
    .text-gold { color: #fcd535 !important; font-weight: bold; }
    .trade-plan-row { margin-top: 16px; padding-top: 16px; border-top: 1px dashed #2b3139; display: flex; justify-content: space-between; align-items: center; }
...
# --- TOP CONTROL BAR ---
col1, col2, col3, col4, col5 = st.columns([1.5, 1.5, 2.5, 1.5, 1.5])

with col1: selected_tf = st.selectbox("⏱️ Timeframe", ["15m", "1h", "4h"], index=1)
with col2: filter_opt = st.selectbox("🔍 Filter Signal", ["All", "BUY", "SELL"])
with col3:
    st.write("")
    use_sentiment = st.checkbox("🧠 Apply Sentiment (Fear & Greed)", value=False)
with col4:
    st.write("")
...
if isinstance(result_data, tuple):
    if len(result_data) == 3:
        data, btc_bullish, sentiment_data = result_data
...
filtered_data = [item for item in data if filter_opt == "All" or filter_opt in item.get('signal', '')]

if not filtered_data:
    st.info(f"⚠️ No '{filter_opt}' setups found on {selected_tf} timeframe right now.")
else:
    for item in filtered_data:
        if "BUY" in item['signal']: card_class, icon, sig_color = "card-buy", "🚀", "text-green"
        elif "SELL" in item['signal']: card_class, icon, sig_color = "card-sell", "🔻", "text-red"
        else: card_class, icon, sig_color = "card-wait", "⏳", "text-gray"
        ...
        reason_html = reason_html.replace("🏆 GOLDEN BUY", "<span class='text-gold'>🏆 GOLDEN BUY</span>")
        ...
        if "BUY" in item['signal'] and item.get('qty', 0) > 0:
            ...
            tsl_fmt = f"₹{item.get('tsl', 0):.6f}" if item.get('tsl', 0) < 50 else f"₹{item.get('tsl', 0):,.2f}"
        ...
        html_content += "</div><div style='height: 15px;'></div>"
